...
app.get('/api/admin/variants', requireAdmin, (req, res) => {
  const rows = db.prepare(`
    SELECT
      v.id, v.name, v.source_urls, v.question_count, v.created_at,
...
app.post('/api/admin/variants', requireAdmin, async (req, res) => {
  try {
    const { name, sourceUrls, questionCount } = req.body || {};
    ...
    const questions = await generateQuestionsFromSources(sourceUrls, qc);

    const id = nanoid(10);
    const now = Date.now();
    db.prepare(`
      INSERT INTO variants (id, name, source_urls, question_count, questions, created_at, generated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, name.trim(), JSON.stringify(sourceUrls), qc, JSON.stringify(questions), now, now);
